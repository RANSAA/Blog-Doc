...
export default defineConfig({
  ...
  //部署到Github站点时，使用“/Blog-Doc/”作为基础路径
  base: '/Blog-Doc/',
  vite: {
    define: {
      'process.env.VITE_BASE_URL': JSON.stringify('/Blog-Doc'), //用于给形如_introduction-iOS.m格式文档中的链接添加BASE URL -> 部署到Github站点时需要添加基础路径为：/Blog-Doc
    },
  },
  ...
  head: [
    ...
    ['link', { rel: 'icon', type: 'image/x-icon', href: '/Blog-Doc/img/favicon.ico' }],
    ['link', { rel: 'stylesheet', href: '/Blog-Doc/css/sidebar-left.css' }],
  ],
  themeConfig: {
    logo: '/img/logo.png',
    socialLinks: [
