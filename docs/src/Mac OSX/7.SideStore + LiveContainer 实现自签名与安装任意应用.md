...
- 方案一：依然是下面的代理使用方案。
- 方案二：使用[Sidestore-ClashMi](https://github.com/tom-snow/Sidestore-ClashMi)与 ClashMi 即可替代 LocalDevVPN / StosVPN 。在ClashMi中配置好Sidestore-ClashMi后就可以同时访问外网和实现 LocalDevVPN / StosVPN的相同功能来刷新签名。阿里云盘中-我的软件/Mac OS X/软件/_iPhone中已经配置好了两个可以使用的文件`Sidestore-ClashMi-Mini.yaml`和`Sidestore-ClashMi.yaml`。如需修改可以查看具体的Sidestore-ClashMi配置文件。
